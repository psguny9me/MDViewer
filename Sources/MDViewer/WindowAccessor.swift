...
struct WindowAccessor: NSViewRepresentable {
    ...
    final class HookView: NSView {
        ...
        /// 타이틀바에서 제목 텍스트 필드를 찾아 더블클릭 제스처를 붙인다.
        /// SwiftUI/AppKit이 타이틀바 뷰를 재생성할 수 있으므로 멱등하게,
        /// 살아있는 필드에 이미 붙어 있으면 아무것도 하지 않는다.
        private func hookTitleDoubleClick() {
            guard let window else { return }
            if let titleField, titleField.window === window { return }   // 이미 후킹됨
