...
- **내장 편집** (⌘E) — 원본 마크다운을 직접 편집. `⌘E`로 프리뷰 ⟷ 편집 전환
  ...
  - **편집기 전체 폭** 토글 (프리뷰 숨김)
  - `⌘S`로 저장(UTF-8). 미저장 변경은 닫기 버튼 •로 표시하고, 창 닫기/앱 종료 시 저장 확인
...
```
Sources/MDViewer/
├── MDViewerApp.swift       # @main, AppDelegate(종료 가드), WindowGroup, 명령(⌘E/⌘S)
...
├── WindowAccessor.swift    # 윈도우 닫기 가드 + 미저장 • 표시 (NSWindowDelegate 가로채기)
